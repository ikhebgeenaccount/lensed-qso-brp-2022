...
FILES = {
    'SMGs, Birkin+2020, z=[1.2-6.1]' : _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'table_a2.fits'),
                                      os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'table_a1.fits')],
    ...
    'SMGs, Da Cunha+2015, z=[1.3-6.1]': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'Cunha.csv')], cols_orig=['ID', 'z_phot'] + COLUMNS[2:],
                                                    open_kwargs={'delimiter': '\t'}),
    # 'ULIRGs, Da Cunha+2010, z=[0.03-0.5]': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'ulirgs_cunha1.csv'),
    #                                     os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'ulirgs_cunha2.csv')], join_col='Galaxy',
    ...
    'Local SFGs, Sun+2020, z=0': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'sun.csv')],
    ...
    'SFGs, Tacconi+2013, z=[1-2.4]': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'PHIBBS_table1.csv'),
                                       os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'PHIBBS_table2.csv')], join_col='Source',
    ...
    # COSMOS is a lot
    # 'COSMOS, Laigle+2016': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'COSMOS2015_Laigle+_v1.1.fits')],
    ...
    'QSOs, Jarvis+2020, z=[0.1-0.2]': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'qsos_jarvis.csv')],
                                                  ...
                                                  open_kwargs={'delimiter': '\t'}),
    # 'Local catalog, Salim+2018' : _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'GSWLC-D2.dat')],
    ...
    #                                                                     (df['logMstar'] > -99)]),
    'AGN, Kakkad+2017, z=1.5': _file_reader([os.path.join(App.config().get(section='PLOTTING', option='context_dir'), 'agns_kakkad.csv')],
