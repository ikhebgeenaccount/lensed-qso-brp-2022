...
def AGN_input_1():
    ...
    for i, row in FILTER_PROPERTIES.iterrows():
        ...
        TEL=tel.upper()
        fil=FILTER_PROPERTIES.filtername.values[i].replace("'", 'prime')
    ...
    for i, row in FILTER_PROPERTIES.iterrows():
        ...
        if pd.isnull(galaxy):
         ...
         #Als je wel een glaxy geeft zet je hem op true wanneer hij er in zit, false als niet
            ...
            if lqso.filter_sed().loc[(lqso.filter_sed()['telescope'] == tel) * (lqso.filter_sed()['filter'] == fil.replace("prime", "'"))].shape[0] > 0:
                print(f"    filters['_{tel}_{fil}']=True")
            else:
